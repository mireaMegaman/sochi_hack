...
def crop(path_to_image: str, yolo_predict) -> numpy.ndarray:
    ...
    crop_img = yolo_predict[0].orig_img[y_min:y_min + h, x_min:x_min + w] # тут все еще массив, а не картинка
    return crop_img
...
def draw_boxes(path_to_image: str, yolo_predict) -> numpy.ndarray:
    ...
    x_max = int(x_max * width)
    y_max = int(y_max * height)
    cv2.rectangle(image, (x_min, y_min), (x_max, y_max), (0, 255, 0), 2) # array here
    return image
